...
OUT_DIR = Path("../tiger_titan_embeddings_no_pathology_report")
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        all_features.append(patch_feats.numpy())
        all_coords.append(patch_coords)
        all_slide_ids.extend([slide_id] * len(patch_coords))


    X = np.stack(X, axis=0)
    y = np.asarray(y, dtype=np.float32)

    X_patch = np.concatenate(all_features, axis=0)
    coords_patch = np.concatenate(all_coords, axis=0)
    slide_ids_patch = np.array(all_slide_ids)

    np.save(OUT_DIR / "X.npy", X)
    np.save(OUT_DIR / "y.npy", y)
    np.save(OUT_DIR / "X_patch.npy", X_patch)
    np.save(OUT_DIR / "coords_patch.npy", coords_patch)
    np.save(OUT_DIR / "slide_ids_patch.npy", slide_ids_patch)
